...
(function (factory) {
    ...
    var SmartWizard = /*#__PURE__*/function () {
        function SmartWizard(element, options) {
            ...
            this.nav = this._getFirstDescendant('.wizard-nav'); // Step anchor elements

            this.steps = this.nav.find('.wizard-link'); // Content container

            this.container = this._getFirstDescendant('.wizard-content'); // Content pages

            this.pages = this.container.children('.wizard-pane'); // Assign options
        ...
        _createClass(SmartWizard, [{
            ...
            value: function _setEvents() {
                ...
                this.main.find('.wizard-next').on("click", function (e) {
                    e.preventDefault();

                    _this._showNext();
                }); // Previous button event

                this.main.find('.wizard-prev').on("click", function (e) {
                    e.preventDefault();

                    _this._showPrevious();
                }); // Keyboard navigation event

                this.main.find('.wizard-reset').on("click", function (e) {
                    e.preventDefault();
            ...
            value: function _createToolbar() {
                ...
                if (this.main.find('.wizard-footer').length > 0) {
                    return null;
                }
                if(this.options.showFooter) {
                    var toolbar = $('<div></div>').addClass('wizard-footer justify-content-between') // Create the toolbar buttons
                    var toolbarLeft = $('<div></div>').addClass('left')
                    var toolbarRight = $('<div></div>').addClass('right')

                    toolbar.append(toolbarLeft, toolbarRight)

                    var btnNext = this.options.toolbarSettings.showNextButton !== false ? $('<button />').text(this.options.lang.next).addClass('button button-solid button-primary wizard-next').attr('type', 'button') : null;
                    var btnPrevious = this.options.toolbarSettings.showPreviousButton !== false ? $('<button />').text(this.options.lang.previous).addClass('button button-solid button-secondary wizard-prev mr-16').attr('type', 'button') : null;
                    var btnReset = this.options.toolbarSettings.showResetButton !== false ? $('<button />').text(this.options.lang.reset).addClass('button button-outline button-primary wizard-reset').attr('type', 'button') : null;
                    toolbarLeft.append(btnPrevious, btnNext); // Add extra toolbar buttons
                    toolbarRight.append(btnReset); // Add extra toolbar buttons
            ...
            value: function _setButtons(idx) {
                ...
                if (!this.options.cycleSteps) {
                    this.main.find('.wizard-prev').removeClass("disabled");
                    this.main.find('.wizard-next').removeClass("disabled");
                    ...
                    switch (this._getStepPosition(idx)) {
                        ...
                        case 'last':
                            this.main.find('.wizard-next').addClass("disabled");
                            break;
                        ...
                        default:
                            ...
                            if (this._getPreviousShowable(idx) === false) {
                                this.main.find('.wizard-prev').addClass("disabled");
                            }
